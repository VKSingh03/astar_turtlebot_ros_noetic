...
class Astar:
    ...
    def __init__(self,width,height,scale,robot_clear,obj_clear,left_rpm,right_rpm):
        
        rospy.init_node('astar_pathgen', anonymous=True)
        self.pub_flag = rospy.Publisher('/startflag', Bool, queue_size=10) 
        ...
        self.OBJ=obj_clear
        self.LEFTRPM = left_rpm
        self.RIGHTRPM = right_rpm
    ...
    def game(self):
        self.pub_flag.publish(Bool(False))
        self.rate.sleep()
        self.running=True
        start=False
        goal=False
        while(start==False):
            sx=float(input('Enter Start X: '))
            sy=float(input('Enter Start Y: '))
            st=float(input('Enter Start Theta: '))
            self.START=(int(sy*100+100),int(sx*100+50),int(st))
        ...
        while(goal==False):
            ...
            gt=float(input('Enter Goal Theta: '))
            self.GOAL=(int(gy*100+100),int(gx*100+50),int(gt))
        ...
        self.pub_flag.publish(Bool(True))
        self.rate.sleep()
...
if __name__ == "__main__":
    ...
    right_rpm=int(input("Enter robot right rpm: "))         
